...
var groceryListContainer = document.getElementById("grocery_list");
...
function addItem() {
  ...
  var itemQuantity = parseInt(document.getElementById("item_quantity").value); // Checking if any input is missing and if the article already exists or not

  if (itemName && !isNaN(itemQuantity) && itemQuantity > 0) {
    var existingItem = groceryListItem.find(function (item) {
    ...
    if (existingItem) {
      ...
      groceryListItem.push({
        name: itemName,
        quantity: itemQuantity
      });
    }

    groceryListItem.sort(function (a, b) {
